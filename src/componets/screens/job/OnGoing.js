...
//         <ActivityIndicator size="large" color="#747EEF" />
//       </View>
//     ) : [...onProgressData, ...ongoingData]
//         .filter(item => item.billingStatus === "paid" && item.isExpired !== true).length === 0 ? (
//       <View style={styles.noDataContainer}>
//         <Image
//           style={styles.imageWidthongoing}
//           source={Nobokking}
...
//       <FlatList
//         data={[
//           ...onProgressData.filter(item => item.billingStatus === "paid" && item.isExpired !== true),
//           ...ongoingData.filter(item => item.billingStatus === "paid" && item.isExpired !== true),
//         ]}
//         keyExtractor={(item, index) => index.toString()}
//         renderItem={({ item }) => (
//           <BookingComp
